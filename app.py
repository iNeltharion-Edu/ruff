...
def create_app():
    ...
    def sglistsingle():
            ...
            if own == "Y":
                ...
                if lowest_comp_price != "NO COMP":
                    ...
                    # positive difference == lower bprice
                        ...
                        """IF THE COMPETITOR IS LOWER THEN THE FV IT WILL SET TO BFLOOR
                        (MEANING WE ARE AT FV BUT WE ARE NOT THE LOWEST ANYMORE)"""

                        """SO THE TEAM WHEN THEY SEE BFLOOR WILL COMPARE TO THE 2ND LOWEST AND -1 TO THEIR APROX BPRICE
                        THEREFORE AVOIDING ANY WEIRD BPRICE FROM"""
        ...
        broaddf = listdf[
            (listdf["tags"].str.contains("donot-broadcast"))
            & (listdf["notes"].str.contains("BG:"))
            & (listdf["tags"].str.contains("for-spin"))
        ]
