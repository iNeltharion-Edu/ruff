...
def countsapi2(tid, sdate, edatestr):
    ...
    def calculate_newfv(price, fees, team):
        """Вычисляет новый коэффициент с учетом команды и цены."""
        if price != "NA":
            allinprice = float(price) + fees
            newfv = str(round((allinprice * 1.2), 2))
            if team in ["Nationals", "Mets", "Angels"]:
                newfv = str(round((allinprice * 1.15), 2))
            return newfv
        return None

    def extract_oldfv(notes):
        """Извлекает старую стоимость из заметок."""
        p = r"[\d]*[.][\d]+|[\d]+"
        match = re.search(p, notes)
        return float(match[0]) if match else None

    def generate_bg_fp_notes(newfv, notes, oldfv, prefix):
        """Генерирует строки с новыми значениями для BG и FP."""
        oldnotes = str(notes).split(str(oldfv))[1] if oldfv is not None else ""
        return f"{prefix}:{newfv}{oldnotes}"

    def process_broadcast(notes, broad, prevbroad, qty, newfv, oldfv, status):
        """Обрабатывает логику с трансляцией."""
        ycount = ["Y"]  # Extract ycount from x
        if broad == "Y" and prevbroad == "":
            if qty == 1:
                return generate_bg_fp_notes(newfv, notes, oldfv, "BG"), "NA", f"{status} Broadcast"
            elif qty > 1:
                return generate_bg_fp_notes(newfv, notes, oldfv, "BG"), "NA", f"{status} Rebroadcast"
        elif broad == "Y" and prevbroad == "1":
            if float(newfv) == oldfv:
                return "NA", "NA", "NO CHANGES"
            elif float(newfv) > oldfv:
                return generate_bg_fp_notes(newfv, notes, oldfv, "BG"), "NA", "Price Increase"
            elif float(newfv) < oldfv:
                return generate_bg_fp_notes(newfv, notes, oldfv, "BG"), "NA", "Price Decrease"
        elif broad == "N" and prevbroad == "1" and ycount == 0:
            return "NA", "NA", "Unbroadcast"
        return "NA", "NA", "NA"

    def process_fp(notes, broad, prevbroad, qty, newfv, oldfv, status):
        """Обрабатывает логику с FP."""
        if broad == "Y" and prevbroad == "":
            if qty == 1:
                return "NA", generate_bg_fp_notes(newfv, notes, oldfv, "FP"), f"{status} Broadcast"
            elif qty > 1:
                return "NA", generate_bg_fp_notes(newfv, notes, oldfv, "FP"), f"{status} Rebroadcast"
        elif broad == "Y" and prevbroad == "1":
            if float(newfv) == oldfv:
                return "NA", "NA", "NO CHANGES"
            elif float(newfv) > oldfv:
                return "NA", generate_bg_fp_notes(newfv, notes, oldfv, "FP"), "Price Increase"
            elif float(newfv) < oldfv:
                return "NA", generate_bg_fp_notes(newfv, notes, oldfv, "FP"), "Price Decrease"
        return "NA", "NA", "NA"

    def notes(x, team):
        # Извлечение данных из x
        notes = x["notes"]
        qty = int(x["quantity"])
        bg = x["BG"]
        fp = x["FP"]
        ...
        newfv = calculate_newfv(price, fees, team)

        if newfv is None:
            return "NA", "NA", "NA"

        # Извлекаем старую стоимость из заметок
        oldfv = extract_oldfv(notes)

        # Обрабатываем BG и FP
        if bg == "Y":
            return process_broadcast(notes, broad, prevbroad, qty, newfv, oldfv, "New")
        elif fp == "Y":
            return process_fp(notes, broad, prevbroad, qty, newfv, oldfv, "New")
        elif (bg == "N" or fp == "N") and price != "NA":
            if broad == "Y" or qty == 1:
                return (
                    generate_bg_fp_notes(newfv, notes, oldfv, "BG"),
                    generate_bg_fp_notes(newfv, notes, oldfv, "FP"),
                    "New Broadcast",
                )
            elif broad == "Y" or qty > 1:
                return (
                    generate_bg_fp_notes(newfv, notes, oldfv, "BG"),
                    generate_bg_fp_notes(newfv, notes, oldfv, "FP"),
                    "Rebroadcast",
        ...
        if ed_count > 1:
            for data in results:
                ...
                if broaddf.empty:
                    ...
                    broaddf = broaddf.loc[broaddf["notes"].str.contains(".com")]
